...
> Quick tip
> 
> You only need to run the `export` lines above once in your shell session. We recommend that you put those lines into a `.env` file, apply it once using `. .env`, and avoid checking it in when committing your code. There's a sample `.env` file [here](https://github.com/ProjectOpenSea/opensea-creatures/blob/master/.env.sample).
